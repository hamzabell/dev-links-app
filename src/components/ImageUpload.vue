<template>
    <div class="image-upload-wrapper" :input="modelValue" :style="{ background: modelValue ? `url(${modelValue})`: '#EFEBFF' }" @click="e => $emit('update:modelValue', e.target.value)" :data-IsImageUploaded="modelValue && modelValue !== ''">
        <svg class="image__icon" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<style lang="scss">
    ...
    .image-upload-wrapper {
        ...
        .image__icon {
            ...
            width:  4rem;
            object-fit: contain;
            content: url('../assets/ph_image.png');
        }

        p {
            color: pp.$purple;
            font-size: pp.$heading-xs;
            font-weight: 500;
            margin-top: .5rem;
        }

        &[data-IsImageUploaded='true'] {
            background-size: cover !important;
            background-repeat: no-repeat !important;

            img {
                content: url('../assets/ph_image-white.png')
            }
        }
...
<docs lang="md">
  ...
  ##### Default Image Upload (No Image)
  ```jsx 
    <ImageUpload />
  ```


  ##### Image Upload with Image Uploaded
  ```jsx 
    <ImageUpload imageUrl="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=687&q=80" />
  ```
</docs>
